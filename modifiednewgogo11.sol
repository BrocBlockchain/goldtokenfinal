...
contract Chargable is AccountTyped {
    ...
    constructor(address _holdingAcct) public {
     ...
     */
    function holdingFeeOwedOnBalance(address _target, uint256 _balance)
        public
        view
...
contract GoldToken is ERC20, ERC20Detailed, Pausable, Redeemable, Chargable {
    ...
    constructor(address _feeHoldingAcct, address _mintHoldingAcct, address _root)
        public
        ERC20Detailed("GoldGo", "GOGO", 18)
         //AccountTyped(msg.sender) //todo recover this
        AccountTyped(_root)
        Chargable(_feeHoldingAcct)
        ...
        returns (bool success)
    {
        ...
        if (getAcctTypeByAddr(_to).name == stringToBytes32("BURN")) {
            createRedeemRequest(minters[uint256(_to)].id, msg.sender, _value);
        }
        collectHoldingFee(msg.sender);
        collectHoldingFee(_to);

        collectTxnFee(msg.sender, _to, _value);
        ...
        returns (bool)
    {
        // TODO: use BURN index instead of name?
        if (getAcctTypeByAddr(_to).name == stringToBytes32("BURN")) {
            createRedeemRequest(minters[uint256(_to)].id, _from, _value);
        }
        collectHoldingFee(_from);
        collectHoldingFee(_to);
        collectTxnFee(_from, _to, _value);
        ...
        returns (uint256 txnFees)
    {
        ...
        if (txnFees != 0) {
            _transfer(_from, feeHoldingAccount, txnFees);
        }
    }
